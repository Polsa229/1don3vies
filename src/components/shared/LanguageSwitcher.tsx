...
export function LanguageSwitcher() {
  ...
  return (
    ...
    >
      <Globe className="w-4 h-4 text-warmgray-400 ml-2 mr-1" aria-hidden="true" />
      {(['fr', 'en'] as const).map((l) => (
        <button
          ...
          className={`px-3 py-1.5 text-sm font-semibold rounded-full transition-all duration-200 ${
            lang === l
              ? 'bg-bordeaux-700 text-ivory-50 shadow-sm'
              : 'text-warmgray-600 hover:text-bordeaux-700 hover:bg-bordeaux-50'
          }`}
        >
          {l.toUpperCase()}
        </button>
