...
const SEEN_KEY = 'hemolink-intro-seen';

/**
 * Animated loading screen shown once per session.
 * Sequence kept short so perceived load stays snappy.
 */
export function LoadingScreen({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    timers.push(setTimeout(() => setStage(2), 450));
    timers.push(setTimeout(() => setStage(3), 1200));
    timers.push(setTimeout(() => setStage(4), 1650));
    timers.push(setTimeout(() => setStage(5), 2200));
    timers.push(
      setTimeout(() => {
        try {
